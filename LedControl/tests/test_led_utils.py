...
def test_led_utils_init(mock_anim_runner, mock_neopixel):
    ...
    led = LED_UTILS(num_days=5, animation=1, none_color=(1,2,3), event_color=(4,5,6), pin_num=18, brightness=0.5)
...
def test_show_weather_calls_anim(mock_anim_runner, mock_neopixel):
    led = LED_UTILS(num_days=5)
    anim = MagicMock()
    led.anim = anim
    led.turn_all_off = MagicMock()
    # Each condition should call the right anim method
    led.show_weather({'condition': 'clear', 'temperature': 10}, brightness=0.5, duration_sec=0.01)
    anim.sun_animation_loop.assert_called()
    led.show_weather({'condition': 'clouds', 'temperature': 10}, brightness=0.5, duration_sec=0.01)
    anim.cloud_animation_loop.assert_called()
    led.show_weather({'condition': 'rain', 'temperature': 10}, brightness=0.5, duration_sec=0.01)
    anim.rain_animation_loop.assert_called()
    led.show_weather({'condition': 'drizzle', 'temperature': 10}, brightness=0.5, duration_sec=0.01)
    anim.rain_animation_loop.assert_called()
    led.show_weather({'condition': 'snow', 'temperature': 10}, brightness=0.5, duration_sec=0.01)
    anim.snow_animation_loop.assert_called()
    led.show_weather({'condition': 'thunderstorm', 'temperature': 10}, brightness=0.5, duration_sec=0.01)
    anim.thunderstorm_animation_loop.assert_called()
    led.show_weather({'condition': 'mist', 'temperature': 10}, brightness=0.5, duration_sec=0.01)
    anim.fog_animation_loop.assert_called()
    led.show_weather({'condition': 'unknown', 'temperature': 10}, brightness=0.5, duration_sec=0.01)
    anim.default_animation_loop.assert_called()
    # Should call display_number for temperature
    led.display_number = MagicMock()
    led.show_weather({'condition': 'clear', 'temperature': 5}, brightness=0.5, duration_sec=0.01)
