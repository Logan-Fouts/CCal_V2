...
@patch('github_tracker.CONFIG_MANAGER')
...
def test_fetch_github_events_basic(mock_get, mock_config_manager, mock_config):
	...
	tracker = GITHUB_TRACKER(num_days=3)
...
def test_get_event_counts_handles_dates():
	tracker = GITHUB_TRACKER(num_days=5)
	now = time.time()
	events = [
		{'created_at': time.strftime('%Y-%m-%dT00:00:00Z', time.gmtime(now))},  # today
		{'created_at': time.strftime('%Y-%m-%dT00:00:00Z', time.gmtime(now - 86400))},  # 1 day ago
		{'created_at': time.strftime('%Y-%m-%dT00:00:00Z', time.gmtime(now - 2*86400))},  # 2 days ago
...
@patch('github_tracker.CONFIG_MANAGER')
@patch('github_tracker.requests.get')
def test_print_new_events_prints_and_updates(mock_get, mock_config_manager, mock_config):
	# Setup
	...
	events = [
		{'id': '999', 'type': 'PushEvent', 'repo': {'name': 'repo1'}, 'created_at': '2025-09-15T00:00:00Z'},
		{'id': '888', 'type': 'PullRequestEvent', 'repo': {'name': 'repo2'}, 'created_at': '2025-09-14T00:00:00Z'},
		{'id': '12345', 'type': 'IssuesEvent', 'repo': {'name': 'repo3'}, 'created_at': '2025-09-13T00:00:00Z'},
	]
...
def test_print_new_events_no_new(mock_get, mock_config_manager, mock_config):
	...
	events = [
		{'id': '12345', 'type': 'PushEvent', 'repo': {'name': 'repo1'}, 'created_at': '2025-09-15T00:00:00Z'},
	]
	mock_get.return_value = MagicMock(status_code=200, json=lambda: events)
	tracker = GITHUB_TRACKER(num_days=3)
	result = tracker.print_new_events()
