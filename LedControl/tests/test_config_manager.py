"""Unit tests for the ConfigManager and WeatherTracker classes in CCal_V2."""
...
data = {"weather": [{"main": "Rain"}], "main": {"temp": 15}}
...
def run_before_each_test(tracker):
    """Automatically update weather before each test."""
    tracker.update_weather("current", data)


def test_update_weather_sets_cache(tracker):
    """Test that update_weather sets the cache and weather data."""
...
def test_get_weather(tracker):
    """Test get_weather returns correct condition and temperature."""
...
def test_load_config_not_dict(tmp_path):
    """Test loading config with non-dict JSON returns empty dict."""
...
def test_load_config_valid(tmp_path):
    """Test loading config with valid dict JSON returns the dict."""
...
def test_save_config_and_load(tmp_path):
    """Test saving and loading config returns the same data."""
...
def test_save_config_error(monkeypatch, tmp_path):
    """Test save_config handles file write errors gracefully."""
...
def test_unquote_basic():
    """Test unquote method decodes percent-encoded strings."""
    cm = ConfigManager()
